...
# above email address.
#
# xpdf ../../EBook/An_Introduction_to_Statistical_Learning.pdf 385 -z 200 & 
#
#-----
...
#
svm.fit = svm( Purchase ~ ., data=OJ[train_inds,], kernel="linear", cost=0.01 )
...
#
tune.out = tune( svm, Purchase ~ ., data=OJ[train_inds,], kernel="linear", ranges=list( cost=c(0.001, 0.01, 0.1, 1, 5, 10, 100, 1000) ) )
...
print( table( predicted=y_hat, truth=OJ[train_inds,]$Purchase ) )
print( sprintf('Linear SVM training error rate (optimal cost=%.2f)= %10.6f',  tune.out$best.model$cost, 1 - sum( y_hat == OJ[train_inds,]$Purchase ) / n_train ) )
...
# 
tune.out = tune( svm, Purchase ~ ., data=OJ[train_inds,], kernel="radial", ranges = list( cost=c(0.001, 0.01, 0.1, 1, 5, 10, 100, 1000), gamma=c(0.5,1,2,3,4) ) )
print( summary(tune.out) ) # <- use this output to select the optimal cost value
